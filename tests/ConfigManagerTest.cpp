...
TEST(ConfigManagerTest, ConfigurationLoading) {
    ...
    // 测试加载配置文件
    EXPECT_TRUE(config.loadConfig("test_config.ini"));
...
TEST(ConfigManagerTest, ConfigurationReload) {
    ...
    EXPECT_TRUE(config.reloadConfig());
    EXPECT_EQ(config.getString("log_level"), "INFO");  // 从文件中重新加载
}
...
TEST(ConfigManagerTest, ConfigurationSaving) {
    ...
    EXPECT_TRUE(config.saveConfig("saved_config.ini"));
    
    // 验证保存的配置
    config.clear();
    EXPECT_TRUE(config.loadConfig("saved_config.ini"));
    EXPECT_EQ(config.getString("save_key"), "save_value");
    EXPECT_EQ(config.getInt("save_int"), 1000);
}
...
TEST(ConfigManagerTest, IniFileParsing) {
    ...
    // 测试解析INI格式的配置文件
    EXPECT_TRUE(config.parseIniFile("test_config.ini"));
